...
function gettingFileInformation(){
    ...
    xhttp.onreadystatechange = ()=>{ // onreadystatechange property takes a callback function
        // if xhttp.readyState === 4 and xhttp.status === 200 then connection is successful
        if(this.readyState === 4 && this.status === 200){ 
            console.log(this.responseText); // xhttp.responseText to see the response in text format
            const fileInfo = document.getElementById('file-info');
            fileInfo.innerHTML = this.responseText;
        }
    }
